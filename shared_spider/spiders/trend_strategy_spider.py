...
class TrendStrategy(RedisSpider):
    name = 'ts_spider'
    redis_key = 'ts:start_urls'
    # start_urls = ['http://stock.stockstar.com/list/strategy.htm']

    def parse(self, response):
        news = response.xpath('//div[@class="listnews"]/ul/li')[:1]
        ...
        if pages and pages[-1].xpath('text()').extract_first() == '下一页':
